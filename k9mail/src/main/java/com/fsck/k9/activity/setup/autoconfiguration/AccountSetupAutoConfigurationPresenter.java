...
import com.fsck.k9.activity.setup.autoconfiguration.AccountSetupAutoConfigurationContract.View;
...
public class AccountSetupAutoConfigurationPresenter implements AccountSetupAutoConfigurationContract.Presenter {
    private ProviderInfo providerInfo;
    private View view;
    private Account account;
    private CheckDirection direction;
    private boolean srvEmailChecked;
    ...
    AccountSetupAutoConfigurationPresenter(View view) {
        this.view = view;
    ...
    public void autoConfiguration(final String email, final String password) {
        if (providerInfo == null) {
            view.setMessage(R.string.account_setup_autoconfiguration);
            new AsyncTask<Void, Void, ProviderInfo>() {
                ...
                protected void onPostExecute(ProviderInfo providerInfo) {
                    ...
                    if (providerInfo != null) {
                        AccountSetupAutoConfigurationPresenter.this.providerInfo = providerInfo;
                        checkSettings(providerInfo, email, password);
                    } else {
                        view.manualSetup(account);
                    }
                }
            }.execute();
        } else {
    ...
    private void modifyAccount(ProviderInfo providerInfo, String email, String password) throws URISyntaxException {
        ...
        Provider provider = Provider.newInstanceFromProviderInfo(providerInfo);

        if (provider.incomingUsernameTemplate.equals(ProviderInfo.USERNAME_TEMPLATE_SRV)) {
            incomingUsername = srvEmailChecked ? userEnc : email;
        } else {
    ...
    @Override
    public boolean checkSettings(ProviderInfo providerInfo, String email, String password) {
        try {
            modifyAccount(providerInfo, email, password);

            return checkSettings();
        } catch (URISyntaxException e) {
            Timber.e("Error while generating account from provider info", e);
            return false;
        }
    }
    ...
    private class CheckAccountTask extends AsyncTask<CheckDirection, Integer, Void> {
        ...
        protected Void doInBackground(CheckDirection... params) {
            try {
                // TODO: 6/27/17 it's hard to handle exception and integrate auto configuration with account checking. I plan to refactor this part using RxJava
                /*
                ...
                clearCertificateErrorNotifications(AccountSetupAutoConfigurationPresenter.this.direction);

                checkServerSettings(AccountSetupAutoConfigurationPresenter.this.direction);

                if (cancelled()) {
                    return null;
                }
                
                if (AccountSetupAutoConfigurationPresenter.this.direction == CheckDirection.INCOMING) {
                    AccountSetupAutoConfigurationPresenter.this.direction = CheckDirection.OUTGOING;
    ...
    private static class Provider implements Serializable {
        ...
        public static Provider newInstanceFromProviderInfo(@Nullable AutoConfigure.ProviderInfo providerInfo) throws
                URISyntaxException {
